...
print_usage (void)
{
	int i;
	printf("\nUsage:\n");
	printf("\tvhash-store <option> <hash-value-hex-string>\n\n");
 ...
 * store_hash
 *
 * Stores a hash in the keystore. Always overwrites any existing value,
 * but the currently in-use hash should have been locked on retrieval
 * at boot time to prevent modification.
 *
 */
static int
store_hash (uint32_t hash_id, char *buf)
{
	ssize_t n = -1;

	n = run_keystore_cmd(KEYSTORE_CMD_STORE, hash_id,
			     KEYSTORE_STORE_OVERWRITE,
			     buf, strlen(buf));
	if (n < 0)
		return 1;
	return 0;
 ...
 * copy_hash
 *
 * Copies and validates the hash value, which should
 * be a 64-character hex string. Returns true if successful,
 * false on an invalid character or incorrect length.
 */
static bool
copy_hash (const char *arg, char *buf, size_t buf_size)
{
	const char *cp;
	size_t i;
	for (cp = arg, i = 0; *cp != '\0' && i < buf_size; cp++, i++) {
		if (!isxdigit(*cp))
			return false;
		buf[i] = *cp;
	}
	if (*cp != '\0' || i != buf_size-1)
		return false;
	buf[i] = '\0';
	return true;
}
...
main (int argc, char * const argv[])
{
	...
	if (!copy_hash(argv[optind], buf, sizeof(buf))) {
		fprintf(stderr, "Error: invalid hash\n");
		print_usage();
		return 1;
	}

	return store_hash(hash_id, buf);
